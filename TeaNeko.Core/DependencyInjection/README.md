...
同时提供 `AddTeaNekoCore()` 一键注册所有核心模块服务的便捷扩展。
...
| `TeaNekoCoreServiceCollectionExtensions` | `AddTeaNekoCore()` — 一键注册所有核心模块 |
...
## 与 AddTeaNekoCore 的关系

`AddTeaNekoCore()` 是手动的模块级注册（注册所有 TeaNeko.Core 内置服务），
`AddTeaNekoAutoScan()` 是注解驱动的自动扫描（扫描用户编写的 `[Service]` 类）。
两者可组合使用：

```csharp
services.AddTeaNekoCore();           // 注册框架内置服务
services.AddTeaNekoAutoScan();       // 自动扫描用户 @Service
```
